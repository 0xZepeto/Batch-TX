...
async function sendNative(network) {
    ...
    console.log('1. Satu akun ke banyak alamat (address.txt)');
    console.log('2. Banyak akun ke satu alamat (received.txt)');
    console.log('3. Kirim semua saldo (split otomatis)');
    const mode = readline.question('Masukkan nomor mode: ');

    if (mode === '1') {
        const addresses = utils.readFile('address.txt');
        if (addresses.length === 0) {
            console.log('Tidak ada alamat di address.txt');
            return;
        }
        ...
        const receiver = utils.readFile('received.txt')[0];
        if (!receiver) {
